...
- [x] T004 [P] Create `pnpm-workspace.yaml` per `plan.md` D1: members `packages/*` and `apps/*`, plus settings `pmOnFail: error`, `engineStrict: true`, `nodeLinker: isolated`. Create **no `.npmrc`** — pnpm 11 reads settings from `pnpm-workspace.yaml` and would silently ignore an `.npmrc` (research R2).
